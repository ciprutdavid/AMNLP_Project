...
class DataCollatorForSeq2SeqMaskLanguageModeling:
    ...
    def mask_tokens(self, inputs: torch.Tensor, mlm_probability=0.15, min_span_length=1, max_span_length=5) -> Tuple[
        torch.Tensor, torch.Tensor]:
        # print(inputs)
        device = inputs.device
        inpts = inputs.clone()
        span_lengths = torch.randint(low=min_span_length, high=max_span_length + 1, size=(inpts.shape[0],),
                                     device=device)
        ...
        masks = torch.stack(
            [(torch.arange(start=0, end=inpts.shape[1]) + offset) % period < span for offset, period, span in
             zip(offsets, periods, span_lengths)])
        ...
        for inpt, mask in zip(inpts, masks):
            new_inpts.append(
                self._noise_span_to_unique_sentinel(inpt, mask, 100,
                                                    tokenizer.convert_tokens_to_ids(['<extra_id_0>'])[0])
            )
            lbls.append(
                self._noise_span_to_unique_sentinel(inpt, ~mask, 100,
                                                    tokenizer.convert_tokens_to_ids(['<extra_id_0>'])[0])
